six_stars_pool = ["Aak", "Angelina", "Archetto", "Bappipe", "Blaze",
                    "Blemishine", "Ceobe", "Ch'en", "Eunectes", "Exusiai",
                    "Eyjafjalla", "Hellagur", "Hoshiguma", "Ifrit", "Magallan",
                    "Mostima", "Mountain", "Mudrock", "Nightingale", "Passenger",
                    "Rosa", "Saga", "Saria", "Schwarz", "Shining",
                    "Siege", "SilverAsh", "Skadi", "Surtr", "Suzuran",
                    "Thorns", "Weedy"]

five_stars_pool = ["Andreana", "Aosta", "Asbestos", "Astesia", "Ayerscarpe",
                    "Beeswax", "Blue Poison", "Broca", "Chiave", "Cliffheart",
                    "Croissant", "Executor", "FEater", "Firewatch", "Franka",
                    "Glaucus", "Greythroat", "Hung", "Iris", "Istina",
                    "Kafka", "Lappland", "Leonhardt", "Liskarm", "Manticore",
                    "Mayer", "Meteorite", "Mr. Nothing", "Nearl", "Nightmare",
                    "Platinum", "Pramanix", "Projekt Red", "Provence", "Ptilopsis",
                    "Reed", "Sesa", "Shamare", "Silence", "Skyfire",
                    "Sora", "Specter", "Swire", "Texas", "Toddifons",
                    "Tsukinogi", "Warfarin", "Whisperain", "Zima"]

four_stars_pool = ["Aciddrop", "Ambriel", "April", "Arene", "Beanstalk",
                "Bubble", "Click", "Coura", "Cutter", "Deepcolor",
                "Dobermann", "Flint", "Frostleaf", "Gitano", "Gravel",
                "Greyy", "Gummy", "Haze", "Jackie", "Jaye",
                "Jessica", "Leizi", "Matoimaru", "Matterhorn", "May",
                "Meteor", "Mousse", "Myrrh", "Myrtle", "Perfumer",
                "Pinecone", "Podenco", "Rope", "Scavenger", "Shaw",
                "Shirayuki", "Sussuro", "Utage", "Vermeil", "Vigna"]

three_stars_pool = ["Adnachiel", "Ansel", "Beagle", "Beehunter", "Cardigan",
                    "Catapult", "Fang", "Hibiscus", "Kroos", "Lava",
                    "Melantha", "Midnight", "Orchid", "Plume", "Popukar",
                    "Spot", "Steward", "Vanilla"]
